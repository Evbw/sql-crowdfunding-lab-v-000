...
"SELECT users.name, SUM(pledges.amount) FROM users JOIN pledges ON projects.id = pledges.project_id GROUP BY users.name ORDER BY(amount) ORDER BY(users.name)"
end

def selects_the_category_names_and_pledge_amounts_of_all_pledges_in_the_music_category
  "SELECT projects.category, SUM(pledges.amount) FROM projects JOIN pledges ON projects.id = pledges.project_id WHERE projects.category = 'music' GROUP BY projects.title"
end
